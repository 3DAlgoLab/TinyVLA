...
class ResNetVisualEncoder(nn.Module):
    ...
    def __call__(self, rgb, *, train):
        ...
        resnet_features = resnet_v1.MultiscaleResNet(
            ...
            rgb, train=False
        )  # train=False since these are frozen.
...
class Add1DPositionEmbedding(nn.Module):
    ...
    def __call__(self, inputs):
        ...
        pos_embedding = sinusoidal_init(max_len=embedding_length)(
            None, pos_emb_shape, None  # pytype: disable=wrong-arg-types  # jax-ndarray
        )
...
def positional_encoding2d(d_model, height, width, flatten=True):
    ...
    div_term = np.exp(np.arange(0.0, d_model, 2) * -(np.log(10000.0) / d_model))
    pos_w = np.expand_dims(np.arange(0.0, width), 1)
    pos_h = np.expand_dims(np.arange(0.0, height), 1)
...
class TemporalTransformer(nn.Module):
    ...
    def __call__(self, x, *, train):
        normal_initializer = jax.nn.initializers.normal(stddev=0.05)
        x = nn.Dense(
            self.d_model, kernel_init=normal_initializer, bias_init=normal_initializer
        )(x)
    ...
    def __call__(self, x, *, train):
        ...
        for _ in range(self.num_layers):
            fused_x = PrenormPixelLangEncoder(
                num_heads=2, dropout_rate=0.1, mha_dropout_rate=0.0, dff=self.d_model
...
class SequenceLAVMSE(nn.Module):
    ...
    def setup(self):
        self.encoder = SequenceLAVAEncoder(
            ...
            temporal_transformer_num_layers=self.lava_temporal_transformer_num_layers,
