...
class ObstacleOrientedPushOracleBoard2dRRT(oriented_push_oracle.OrientedPushOracle):
    ...
    def _get_obstacle_poses(self, raw_state):
        ...
        obstacle_poses = [
            i for i in obstacle_poses if np.max(i) < BEYOND_TABLE_THRESHOLD
        ]
    ...
    def get_plan(self, raw_state):
        ...
        xy_target = xy_ee + raw_state["effector_target_to_task_target_translation"][:2]

        # Get position of task block. This is the one we push.
        xy_block = xy_ee + raw_state["effector_target_to_start_block_translation"][:2]
        ...
        obstacle_poses = [
            i for i in obstacle_poses if np.linalg.norm(xy_block - i) > EPS
        ]

        # If the xy_target is a block center of mass, we are doing 'block2block',
        # so remove it from the list of obstacles.
        obstacle_poses = [
            i for i in obstacle_poses if np.linalg.norm(xy_target - i) > EPS
        ]
        ...
        if not success:
            logging.info(
                "RRT failed, likely because the environment was initialized"
        ...
        if self._logging:
            logging.info(
                "Filtered block subgoal path from %d to %d", len_p_before, len_p_after
            )
    ...
    def _maybe_advance_to_next_subgoal(self, info, raw_state):
        ...
        if info.distance_to_target <= ADVANCE_TO_NEXT_SUBGOAL_THRESHOLD and self._plan:
    ...
    def _get_move_to_preblock_avoid(self, info, raw_state):
        ...
        if self._use_ee_planner and self._ee_plan_success:
            ...
            xy_delta, max_step_velocity = self._get_move_to_preblock_avoid_potential(
                info
            )
        return info, xy_delta, max_step_velocity

    def _get_action_for_block_target(self, raw_state, block="block", target="target"):
        ...
        if "instruction" in raw_state:
            ...
            if np.linalg.norm(self._prev_instruction - curr_instruction) > 0.0:
                self.reset()
            self._prev_instruction = curr_instruction
        ...
        if apply_slowdown:
            ...
            if self._logging:
                logging.info(
                    "subgoal %d, info.distance_to_target: %.3f, "
                    "max_step_before: %.3f, max_step_after: %.3f",
                    len(self._plan),
                    info.distance_to_target,
                    before_max_step_distance,
                    max_step_distance,
                )
    ...
    def _get_filtered_ee_obstacle_poses(self, obstacle_poses, xy_target, pushing_block):
    ...
    def _get_move_to_preblock_avoid_potential(self, info):
        ...
        pf_delta = np.array([0.0, 0.0])

        for pose_xy in info.obstacle_poses:
            potential = calc_repulsive_potential(
                info.xy_ee[0], info.xy_ee[1], pose_xy[0], pose_xy[1]
            )
            pf_delta += potential
        ...
        for pose_xy in [info.xy_pre_block]:
            potential = calc_attractive_potential(
                info.xy_ee[0], info.xy_ee[1], pose_xy[0], pose_xy[1]
            )
            pf_delta += potential
    ...
    def _get_action_info(self, raw_state, unused_block=None, unused_target=None):
        ...
        xy_task_target = (
            xy_ee + raw_state["effector_target_to_task_target_translation"][:2]
        )
