...
class LanguageTableReward(object):
    ...
    def _sample_objects(self, blocks_on_table):
        ...
        start_block, target_block = self._rng.choice(blocks_on_table, 2, replace=False)
        return start_block, target_block


def target_inside_bounds(target, buffer=constants.WORKSPACE_BOUNDS_BUFFER):
    target_x, target_y = target
    return (
        target_x > constants.X_MIN + buffer
        and target_x < constants.X_MAX - buffer
        and target_y > constants.Y_MIN + buffer
        and target_y < constants.Y_MAX - buffer
    )
