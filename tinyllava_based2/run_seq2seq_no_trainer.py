...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=2000,
        help=(
            "Number of training examples to use. If None, all training examples will be used."
        ),
    )
    parser.add_argument(
        "--n_val",
        ...
        default=500,
        help=(
            "Number of validation examples to use. If None, all validation examples will be used."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            "The name of the experiment tracking folder. Only applicable when `--with_tracking` is passed."
        ),
    )
    ...
    for step, batch in enumerate(eval_dataloader):
        with torch.no_grad():
            ...
            if count_printed < args.n_val_batch_generations:
                logger.info(
                    "printing few sample generations and corresponding labels from eval set"
                )
                logger.info("prompt | generated | label")
                decoded_prompts = tokenizer.batch_decode(
                    batch["input_ids"], skip_special_tokens=False
                )
                for prompt, generated_response, response in zip(
                    decoded_prompts, decoded_preds, decoded_labels
                ):
    ...
    if args.dataset_name is not None:
        ...
        if args.n_train > 0:
            raw_datasets["train"] = datasets.Dataset.from_dict(
                raw_datasets["train"][: args.n_train]
            )
        if args.n_val > 0:
            raw_datasets["validation"] = datasets.Dataset.from_dict(
                raw_datasets["validation"][: args.n_val]
            )
    else:
        data_files = {}
        if args.train_file is not None:
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if isinstance(checkpointing_steps, int):
                if completed_steps % checkpointing_steps == 0:
                    ...
                    if accelerator.state.deepspeed_plugin is not None:
                        checkpoint_model(
                            args.output_dir, epoch, model, epoch, completed_steps
                        )
                    else:
                        accelerator.wait_for_everyone()
                        if accelerator.is_main_process:
                            ...
                            accelerator.save(
                                model.state_dict(), os.path.join(ckpt_path, "model.pt")
                            )
    ...
    if args.load_best_model:
        if accelerator.state.deepspeed_plugin is not None:
            ...
            model.load_state_dict(
                torch.load(
                    os.path.join(best_metric_checkpoint, "model.pt"),
