...
_CHECKPOINT_PATH = flags.DEFINE_string("checkpoint_path", None, "FLAX checkpoint path.")
...
def evaluate_checkpoint(checkpoint_path, workdir, config):
    ...
    for reward_name, reward_factory in rewards.items():
        ...
        for ep_num in range(num_evals_per_reward):
            ...
            # init is valid.
            oracle_policy = (
                push_oracle_rrt_slowdown.ObstacleOrientedPushOracleBoard2dRRT(
            ...
            video_path = os.path.join(
                workdir, "videos/", f"{reward_name}_{ep_num}_{success_str}.mp4"
            )
