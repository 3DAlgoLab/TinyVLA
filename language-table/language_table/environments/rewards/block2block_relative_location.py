        ...
        for verb in BLOCK2BLOCKRELATIVELOCATION_VERBS:
            for direction in DIRECTIONS:
                for direction_syn in DIRECTION_SYNONYMS[direction]:
                    # # Add instruction.
                    inst = f"{verb} {block_syn} {direction_syn} {target_block_syn}"
...
class BlockToBlockRelativeLocationReward(base_reward.LanguageTableReward):
    ...
    def __init__(self, goal_reward, rng, delay_reward_steps, block_mode):
        super(BlockToBlockRelativeLocationReward, self).__init__(
    ...
    def instruction_for(self, pushing_block, target_block, direction, blocks_on_table):
    ...
    def get_distance_for(self, state, pushing_block, target_block, direction):
        ...
        pushing_block_translation, _ = self._get_pose_for_block(pushing_block, state)
        # Get current relative location of target.
        target_translation = self.target_translation_for(state, target_block, direction)
        # Compute distance between current translation and target.
        dist = np.linalg.norm(
            np.array(pushing_block_translation) - np.array(target_translation)
        )
        return dist
    ...
    def debug_info(self, state):
        """Gets distance between pushing and target block. Useful for debugging."""
        # Get current location of pushing block.
        pushing_block_translation, _ = self._get_pose_for_block(self._block, state)
        # Get current relative location of target.
        ...
        # Compute distance between current translation and target.
        dist = np.linalg.norm(
            np.array(pushing_block_translation) - np.array(target_block_translation)
        )
        return dist

    def reward_for(
        self, state, pushing_block, target_block, direction, delay_reward_steps
    ):
        """Gets reward for a pushing block and a relative location."""
        pushing_block_translation, _ = self._get_pose_for_block(pushing_block, state)
        target_block_translation, _ = self._get_pose_for_block(target_block, state)
        # Consider the end point of the line 2x longer than the offset.
        offset_translation = self.target_translation_for(state, target_block, direction)
        ...
        amount_dragged = np.linalg.norm(
            self._target_block_reset_translation - target_block_translation
        )
