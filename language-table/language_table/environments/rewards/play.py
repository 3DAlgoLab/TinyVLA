    ...
    for b in blocks:
        for l0 in locations:
            for l1 in locations:
                if l0 != l1:
                    inst = (
                        "put the %s in the %s, then put the rest of the "
        ...
        for l0 in locations:
            for l1 in locations:
                if l0 != l1:
                    ...
                    inst = "put %s %s in the %s, then put the " "rest in the %s" % (
                        number,
                        block,
                        l0,
                        l1,
...
def triangle_in_place_remainder_in_rest(locations):
    ...
    for l0 in locations:
        for l1 in locations:
            if l0 != l1:
                inst = (
                    "make a triangle out of three blocks and put it "
...
@gin.configurable
def get_100_4block_instructions(
    num_train_per_family=20, num_test_per_family=5, return_train=True
):
    ...
    def shuffle_split(inst_list):
        ...
        if num_train_per_family:
            return (
                ...
                inst_list[
                    num_train_per_family : (num_train_per_family + num_test_per_family)
                ],
            )
        else:
            return inst_list, []
    ...
    train_inst, test_inst = accum_train_test(combo0_instructions, train_inst, test_inst)
    combo1_instructions = obj_in_place_then_remainder_in_other(BLOCKS4, LOCATIONS)
    train_inst, test_inst = accum_train_test(combo1_instructions, train_inst, test_inst)
    combo2_instructions = k_in_place_then_k_minus_1_in_other(BLOCKS4, LOCATIONS)
    train_inst, test_inst = accum_train_test(combo2_instructions, train_inst, test_inst)
    combo3_instructions = triangle_in_place_remainder_in_rest(LOCATIONS)
    train_inst, test_inst = accum_train_test(combo3_instructions, train_inst, test_inst)
    combo4_instructions = order_by(ORDERINGS)
    train_inst, test_inst = accum_train_test(combo4_instructions, train_inst, test_inst)
    if return_train:
        return train_inst
    else:
        return test_inst
...
def get_random_8block_instruction(rng):
    ...
    all_task_fns = [
        ...
        group_color_pairs,
        get_colors_in_lines,
        group_color_pairs_in_locations,
        get_line_tasks,
        get_surround_tasks,
        blocks_in_order_outer_edge,
        all_blocks_in_location,
        k_blocks_in_location_i_rest_in_location_j,
...
class PlayReward(base_reward.LanguageTableReward):
    ...
    def __init__(self, goal_reward, rng, delay_reward_steps, block_mode):
        super(PlayReward, self).__init__(
