...
def get_shortest_path_no_collisions(
    xy_start,  # (x,y) of moving object.
...
class RRTStar:
    ...
    def __init__(
        ...
        if plot_debug_image:
            self.plotting = plot_module.PlotRRT(x_start, x_goal, boundary, obstacles)
    ...
    def plan(self):
        ...
        if inside_obstacles(self.s_start, self.delta, self.obstacles):
            logging.info(
                "RRT-* failed to find a solution because x_start "
        ...
        for _ in range(self.iter_max):
            ...
            if node_new and not is_collision(
                node_near, node_new, self.delta, self.obstacles, self.boundary
            ):
    ...
    def get_random_node(self, goal_sample_rate):
        if np.random.random() > goal_sample_rate:
            return Node(
                (
                    np.random.uniform(
                        self.x_range[0] + self.delta, self.x_range[1] - self.delta
                    ),
                    np.random.uniform(
                        self.y_range[0] + self.delta, self.y_range[1] - self.delta
                    ),
                )
            )
    ...
    def find_nearest_neighbors(self, node_new):
        ...
        dist_table_index = [
            ...
            and not is_collision(
                node_new, self.vertex[ind], self.delta, self.obstacles, self.boundary
            )
        ]
        # pylint: enable=g-complex-comprehension
...
def is_collision(start, end, delta, obstacles, obs_boundary):
    ...
    end_in_obs = is_inside_obstacles_or_boundary(end, delta, obstacles, obs_boundary)
