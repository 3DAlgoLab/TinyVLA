...
def create_datasets(
    ...
    def _pad_episode(episode, padding):
        ...
        first_item_mid_tensor = tf.nest.map_structure(tf.identity, first_item_tensor)
        first_item_mid_tensor[rlds.IS_FIRST] = False
        padding_ds = tf.data.Dataset.from_tensors(first_item_mid_tensor).repeat(padding)
    ...
    train_preprocess = (
        preprocess_spec.PreprocessFn(  # pytype: disable=wrong-arg-types  # re-none
    ...
    def _preprocess_fn(example_index, features):
        ...
        features[preprocess_spec.SEED_KEY] = tf.random.experimental.stateless_fold_in(
    ...
    obs_statistics = tf.nest.pack_sequence_as(observation_spec, flat_obs_statistics)
...
class DecodeAndRandomResizedCrop(preprocess_spec.RandomMapTransform):
    ...
    def _transform(self, features, seed):
        ...
        if self.random_crop_factor is None:
            ...
            random_crop_factor = tf.constant(self.random_crop_factor, dtype=tf.float32)
...
def _load_normalization_info(norm_checkpoint_path):
    ...
    if "obs.json" not in checkpoint_contents or "act.json" not in checkpoint_contents:
        return None
