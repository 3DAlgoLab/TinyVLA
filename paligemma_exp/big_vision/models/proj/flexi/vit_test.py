...
class PatchEmbTest(absltest.TestCase):
    ...
    def _test_patch_emb_resize(self, old_shape, new_shape, n_patches=100):
        ...
        patch_resized = tf.image.resize(
            tf.constant(patches), resized_patch_shape, method="bilinear"
        ).numpy()
        ...
        new_embeddings = jax.lax.conv_general_dilated(
            patch_resized,
            w_emb_resampled,
            window_strides=resized_patch_shape,
            padding="VALID",
            dimension_numbers=("NHWC", "HWIO", "NHWC"),
            precision="highest",
        )
        ...
        np.testing.assert_allclose(old_embeddings, new_embeddings, rtol=1e-1, atol=1e-4)
