...
def load(params, path, model_cfg=None, dont_load=()):
    ...
    if gfile.exists(f"{checkpoint_path}.index"):
        ...
        max_len = params["BertEncoder_0"]["embedder"]["embedders_position_ids"][
            "embedding"
        ].shape[0]
        ...
        bert_params["embedder"]["embedders_position_ids"]["embedding"] = bert_params[
            "embedder"
        ]["embedders_position_ids"]["embedding"][:max_len]
        return common.merge_params({"BertEncoder_0": bert_params}, params, dont_load)

    logging.info(
        "Could not find original BERT checkpoint path '%s', "
        "loading big_vision checkpoint '%s'",
        checkpoint_path,
        path,
    )
