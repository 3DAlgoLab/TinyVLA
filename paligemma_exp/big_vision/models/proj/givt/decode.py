...
def generate(
    ...
    *,
    ...
    assert (
        labels is not None or batch_size
    ...
    cache, prefill_logits = _create_cache(labels, model, init_sequence, params, encoded)
    ...
    sample = functools.partial(_sample_gmm, cfg_inference_weight=cfg_inference_weight)
    ...
    def tokens_to_logits(tokens, cache, uncond=False):
        ...
        def decode_step(model, tokens):
            return model.decode(
                tokens, labels, encoded, decode=True, drop_labels=drop_labels
            )
    ...
    def sampling_iteration(i, state):
        ...
        # (b, nb, d)
        cur_logits = _unflatten_samples_dim(cur_logits, batch_size, beam_size).squeeze(
            axis=2
        )
        ...
        if cfg:
            ...
            cur_logits_u = _unflatten_samples_dim(
                cur_logits_u, batch_size, beam_size
            ).squeeze(axis=2)
