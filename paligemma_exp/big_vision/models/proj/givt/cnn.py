...
def _dsample(x):
    return _tensorflow_style_avg_pooling(x, (2, 2), strides=(2, 2), padding="same")
...
class Model(vae.Model):
        ...
        *,
        ...
        def _space_to_depth(z):
            ...
            return einops.rearrange(
                z, "b (h ph) (w pw) c -> b (h w) (c ph pw)", ph=ph, pw=pw
    ...
    def decode(self, x: jax.Array, train: bool = False) -> jax.Array:
        ...
        x = einops.rearrange(
            x, "b (h w) (c ph pw) -> b (h ph) (w pw) c", h=h, w=w, ph=ph, pw=pw
        )  # (2, 16, 16, 32)
...
def load(
    ...
    if malib_ckpt:
        ...
        vaegan_params_flat = [
            (k.replace(prefix_old, "cnn_"), v)
            for k, v in vaegan_params_flat
            if prefix_old in k
        ]
