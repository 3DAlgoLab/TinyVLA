...
def get_pp_render_text(
    image_size: int,
    font_size: int = 16,
    max_chars=768,
    background_brightness=127,
    text_brightness=0,
    lower=True,
    monospace=False,
    spacing=1,
    ...
    def trim_letter(letter):
        ...
        first = tf.cond(
            no_pixels, lambda: tf.constant(0, tf.int64), lambda: has_pixels[0]
        )
        last = tf.cond(
            no_pixels, lambda: tf.constant(0, tf.int64), lambda: has_pixels[-1]
        )
    ...
    def to_image(rendered, width, height=None):
        ...
        row_idx = tf.RaggedTensor.from_value_rowids(
            tf.range(tf.shape(rendered)[0]), row_idx
        )
    ...
    def render(text):
        ...
        shifted = tf.bitwise.right_shift(repl, tf.bitwise.bitwise_xor(moded, sevens))
        ...
        if monospace:
            ...
            signature = tf.RaggedTensorSpec(
                shape=(None, font_size), ragged_rank=1, dtype=letters.dtype
            )
