...
def tokenize_constant(model, text, bos="no", eos="no", length=None):
    ...
    tokens = tokenizer.to_int(text, bos=bos == "yes", eos=eos in ("yes", "sticky"))
    ...
    if len(tokens) > length:
        ...
        return np.pad(
            tokens, [(0, length - len(tokens))], constant_values=tokenizer.pad_token
        )
