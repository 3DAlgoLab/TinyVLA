...
def encode_to_codebook_indices(checkpoint, masks):
    ...
    x = _conv_tf(checkpoint, x, strides=1, padding="SAME", layer_name="encoder.10")
...
def _resblock_tf(checkpoint, x, layer_name):
    ...
    original_x = x
    x = _conv_tf(checkpoint, x, padding="SAME", strides=1, layer_name=layer_name + ".0")
    x = tf.nn.relu(x)
    x = _conv_tf(checkpoint, x, padding="SAME", strides=1, layer_name=layer_name + ".2")
    x = tf.nn.relu(x)
    x = _conv_tf(checkpoint, x, padding="SAME", strides=1, layer_name=layer_name + ".4")
    return x + original_x
