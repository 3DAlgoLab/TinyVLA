...
class Vqa(tfds.core.GeneratorBasedBuilder):
    ...
    def _generate_examples(self, split, image_folder=None):
        ...
        with open(
            os.path.join(_VQAV2_PATH, f"v2_OpenEnded_mscoco_{split}_questions.json")
        ) as f:
        ...
        if "test" not in split:
            with open(
                os.path.join(_VQAV2_PATH, f"v2_mscoco_{split}_annotations.json")
            ) as f:
        ...
        for ex in examples:
            ...
            if "test" not in split:
                ...
                ex["answer_confidences"] = [
                    a["answer_confidence"] for a in ann["answers"]
                ]
                ...
                ex["image/filename"] = fname
                if os.path.isfile(
                    path := os.path.join(_IMAGE_PATH, image_folder, fname)
                ):
                    ex["image"] = path
                else:
                    print(ex["image/id"])
