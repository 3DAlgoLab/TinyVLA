...
def main(argv):
    ...
    if jax.process_index() == 0:
        for name, params in params_cpu.items():
            ...
            mw.measure(
                f"num_params_{name}", sum(p.size for p in jax.tree_leaves(params))
            )
    ...
    def loss_fn(student_params, params, data, rngs):
        ...
        def fwd(name, params):
            return jax.named_call(models[name].apply, name=name)(
                {"params": params},
                getfirst(data, name, "image"),
                train=name == "student",
                rngs=rngs.get(name),
            )[
                0
            ]  # logits, unused_outputs
    ...
    @functools.partial(jax.jit, donate_argnums=(0, 1, 2), out_shardings=repl_sharding)
    def update_fn(params, opt, rng, data):
        ...
        if config.get("mixup") and config.mixup.p:
            to_mix = {
                name: data[name]
                for name in ("image", "labels") + tuple(models)
                if name in data
            }
    ...
    for name in config.teachers:
        ...
        params_cpu[name] = get_model_mod(name).load(
            params_cpu[name], init_def, config[name], **config.get(f"{name}_load", {})
        )
    ...
    def teacher_ensemble_fwd(train_state, batch):
        ...
        return (
            jnp.mean([jax.nn.softmax(l) for l in all_teacher_logits], axis=0),
            {},
