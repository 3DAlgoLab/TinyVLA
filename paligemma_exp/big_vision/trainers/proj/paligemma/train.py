...
def main(argv):
    ...
    if resume_ckpt_path:
        ...
        write_note(f"Initialize model from {config.get('model_init') or 'scratch'}...")
    ...
    def update_fn(train_state, rng, batch):
        ...
        def loss_fn(params):
            ...
            if off_value > 0:
                ...
                targets = jnp.where(targets == 1.0, 1.0 - off_value, off_value / denom)
        ...
        if (
            ...
            for k in ("training_loss", "l2_params", "l2_grads"):
                if not np.isfinite(measurements.get(k, 0.0)):
                    raise RuntimeError(
                        f"{k} became nan or inf somewhere within steps "
