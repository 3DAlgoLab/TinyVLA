...
def main(argv):
    ...
    ntrain_img = input_pipeline.get_num_examples(
        config.dataset, config.train_split, data_dir=fillin(config.get("dataset_dir"))
    )
    ...
    for step, train_batch in zip(range(first_step + 1, total_steps + 1), train_iter):
        mw.step_start(step)
        ...
        if (
            u.itstime(step, config.log_training_steps, total_steps, host=0)
            or chrono.warmup
