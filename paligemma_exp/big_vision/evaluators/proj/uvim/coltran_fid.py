...
def _preprocess(image, resolution=512):
    ...
    image = tf.image.resize(
        image, method="area", antialias=True, size=(resolution, resolution)
    )
...
class Evaluator:
    ...
    def __init__(
        ...
        logging.log(
            logging.INFO,
            "Colorizing with batch size %i on %i devices.",
    ...
    def run(self, params):
        ...
        for i, batch in enumerate(self.eval_data.as_numpy_iterator()):
            ...
            y = np.clip(np.round((y + 1.0) * 128.0), 0, 255)
            y = _normalize(y)
            ...
            logging.log_every_n_seconds(
                logging.INFO,
                "ColTran FID eval: processed %i colorized examples so far.",
                30,
        ...
        for i, batch in enumerate(self.reference_data.as_numpy_iterator()):
            ...
            logging.log_every_n_seconds(
                logging.INFO,
                "ColTran FID eval: processed %i reference examples so far.",
                30,
