...
def training(input_config):
    ...
    for name, dataset, weight, total in pool.map(
        ...
        _make,
        ((name, w) for name, w in input_config.data.items() if w),
    ):
        names.append(name)
    ...
    logging.info(
        ...
        sum(totals),
        "\n".join(
            f"{ds}: {n} ({w * 100:.1g}%)" for ds, n, w in zip(names, totals, weights)
        ),
    )
...
def make_for_inference(
    ...
    data = data.cache() if cache_raw else data
    data = data.map(
        _add_internal_fields(preprocess_fn), num_parallel_calls=num_parallel_calls
    )
...
def tf_to_numpy(x):
    ...
    rows = [
        tf_to_numpy(x.values[splits[i] : splits[i + 1]]) for i in range(len(splits) - 1)
    ]
