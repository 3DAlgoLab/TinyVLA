...
class OptaxTest(parameterized.TestCase):
    ...
    def test_get_count(self):
        params = jax.tree.map(jnp.array, {"a": 1.0})
        tx = optax.masked(
    ...
    def test_replace_frozen(self):
        ...
        chex.assert_trees_all_equal(
            bv_optax.replace_frozen(schedule, params, 0.0),
            {"Dense_0": {"kernel": 1.0, "bias": 0.0}},
        )  # pyformat: disable
    ...
    def test_make_wd(self):
        ...
        wds = jax.tree.map(
            jnp.array,
            {
                "Dense_0": {"kernel": 2e-3, "bias": 5e-4, "other": 0.0},
            },
        )  # pyformat: disable
    ...
    def test_make_clip_norm(self):
        ...
        for step in range(total_steps):
            ...
            make_tx = lambda sched: lambda g: -sched * config.lr * g_scale * g
            chex.assert_trees_all_close(
                updates, jax.tree.map(make_tx(sched), grads_scaled)
            )
    ...
    def test_make_multi(self):
        params = jax.tree.map(
            jnp.array,
            {
                ...
                "Dense_3": {"kernel": 10.0, "bias": 11.0, "other": 12.0},
            },
        )  # pyformat: disable
        ...
        wds = jax.tree.map(
            jnp.array,
            {
                "Dense_0": {"kernel": wd1 * wdb, "bias": wd2 * wdb, "other": 0.0},
                "Dense_1": {"kernel": wd1 * wdb, "bias": wd2 * wdb, "other": 0.0},
                "Dense_2": {"kernel": wd1 * wdb, "bias": wd2 * wdb, "other": 0.0},
                "Dense_3": {"kernel": 0.0 * wdb, "bias": 0.0 * wdb, "other": 0.0},
            },
        )  # pyformat: disable
        ...
        total_steps = 10
        sched_kw = dict(global_batch_size=1, total_steps=total_steps)
        tx, (sched_fn1, sched_fn2) = bv_optax.make(config, params, sched_kw=sched_kw)
        opt_state = tx.init(params)

        # Manually specify schedules for computing expected values.
        frozen_fn = lambda _: jnp.array(0.0)
        sched_fns = {
        ...
        gflat, _ = jax.tree.flatten(
            ...
            jax.tree.map(
                lambda g, sched_fn: {frozen_fn: 0}.get(sched_fn, g), grads, sched_fns
            )
        )
        l2_g = jnp.sqrt(sum([jnp.vdot(p, p) for p in gflat]))
        grad_clip_factor = jnp.minimum(1.0, config.grad_clip_norm / l2_g)
        ...
        for step in range(total_steps):
            ...
            np.testing.assert_almost_equal(
                sched2, mult2 * (total_steps - step) / total_steps
            )
            chex.assert_trees_all_close(
                updates,
                jax.tree.map(
    ...
    def test_frozen_no_state(self):
        ...
        nbytes = sum(jax.tree.flatten(jax.tree.map(lambda x: x.nbytes, adam_state))[0])
    ...
    def test_adafactor(self):
        ...
        n_state_params = sum(
            jax.tree.flatten(
                jax.tree.map(
                    lambda x: np.prod(x.shape if hasattr(x, "shape") else 0),
                    adafactor_state,
                )
            )[0]
        )
