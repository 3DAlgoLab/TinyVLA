...
def dist(student, teacher, kind, feat_axis=-1, epsilon=1e-12, t=1, ls=0.0, k=1):
    ...
    if kind == "euclidean":
        ...
        return u.softmax_xent(
            logits=student, labels=pl, reduction=False, kl=True, axis=feat_axis
        )
    elif kind == "kl":
        ...
        logsoftmax_diff = jax.nn.log_softmax(
            student, axis=feat_axis
        ...
        return (
            get_top_k(student, k, feat_axis) == get_top_k(teacher, 1, feat_axis)
        ).sum(feat_axis)
    else:
