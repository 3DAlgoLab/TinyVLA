...
def prepare_datasets(
    ...
    *,
    ...
    ds_texts = (
        tf.data.Dataset.from_tensor_slices(list(class_names))
        .enumerate()
        .flat_map(expand_aliases)
        .flat_map(add_prompts)
        .map(substitute_prompt)
        .map(pp_builder.get_preprocess_fn(f"{pp_txt}|keep('label', 'labels')"))
    )
...
def _split_and_batch(dataset_name, data_dir, class_names, batch_size, split, get_ds):
    """Splits dataset, calls `get_ds` and returns padded + batched datasets."""
    assert (
        not batch_size % jax.device_count()
...
def _average_embeddings(embeddings, *, labels, num_classes, normalize):
    ...
    assert len(labels) == len(embeddings), f"Expected {len(labels)}=={len(embeddings)}"
...
class Evaluator:
    ...
    def __init__(
        ...
        *,
        ...
        def count_correct(train_state, return_embeddings, *, mask, labels, image, ztxt):
        ...
        self._count_correct_p = jax.jit(
            count_correct,
            static_argnums=(1,),
            out_shardings=NamedSharding(self.mesh, P()),
        )
        self._count_p = jax.jit(jnp.sum, out_shardings=NamedSharding(self.mesh, P()))
    ...
    def _embed_texts(self, train_state, dataset_name):
        ...
        for batch in ds_b:
            ...
            embeddings.append(
                jax.device_get(self._embed_texts_p(train_state, batch["labels"]))
            )
            for name in data:
        ...
        logging.info(
            "Embedded %s text in %d steps - ...%s", dataset_name, len(ns), ns[-10:]
        )
        logging.info("Totalling %d text in %.1fs", n, time.monotonic() - t0)
        logging.info(
            "Total texts embeddings size %.1fM", data["embedding"].nbytes / 1e6
        )
        return data

    def evaluate(self, train_state, dataset_name, *, return_embeddings=False):
        ...
        for batch in ds_b:
            ...
            if self._count_correct_p not in self._compiled:
                logging.info(
                    "Compiled image embeddings in %.1fs", time.monotonic() - t0
                )
                t0 = time.monotonic()
        ...
        logging.info(
            "Embedded %s image in %d steps - ...%s", dataset_name, len(ns), ns[-10:]
        )
        ...
        if return_embeddings:
            ...
            data = {k: np.concatenate(v, axis=0) for k, v in data.items()}
            logging.info(
                "Total images embeddings size %.1fM", data["embedding"].nbytes / 1e6
            )
            mask = data.pop("mask").astype(bool)
