...
class Evaluator:
    ...
    def __init__(
        self, predict_fn, tokenizer, model="oi", det_ious=(0.5, 0.75), *, devices, **kw
    ):
    ...
    def run(self, train_state):
        ...
        for _, batch in zip(range(self.steps), self.get_data_iter()):
            ...
            for i, dec in enumerate(decoded):
                ...
                t = np.r_[
                    dec[:4] - self.loc0, dec[4 : 4 + 16] - self.seg0
                ]  # Ignore rest
                if (
            ...
            for v, m64, gtm, bb, gtbb in zip(valid, mask64, gt_masks, bbox, gt_bbs):
                ...
                if not v or x2 <= x1 or y2 <= y1:  # Can still happen after int().
                    ...
                    mi = np.asarray(
                        PIL.Image.fromarray(m64).resize(
                            [x2 - x1, y2 - y1],
                            resample=PIL.Image.BILINEAR,  # pytype: disable=module-attr
                        )
