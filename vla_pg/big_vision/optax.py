...
def find_states(opt_state, cls):
    leaves = jax.tree.leaves(opt_state, is_leaf=lambda node: isinstance(node, cls))
...
def get_count(opt_state, jittable=False):
    ...
    counts = [
        state.count for state in find_states(opt_state, optax.ScaleByScheduleState)
    ]
    if jittable:
        return counts[0]
    else:
...
def replace_frozen(schedule, pytree, replacement, log=None):
    ...
    return jax.tree.map(lambda v, f: replacement if f else v, pytree, frozen_mask)
...
def make(config, params, *, sched_kw):
    ...
    # Gradient clipping.
    grad_clip_norm_tx = (
        optax.masked(optax.clip_by_global_norm(config.grad_clip_norm), not_frozen_mask)
    ...
    if config.get("lr_mults"):
        ...
        assert all(
            mult > 0 for mult in mults
        ), f"Use schedule=None for parameter freezing instead of lr_mults={mults}"
        lr_mult_txs += [
            optax.masked(optax.scale(mult), mask) for mult, mask in zip(mults, masks)
        ]
