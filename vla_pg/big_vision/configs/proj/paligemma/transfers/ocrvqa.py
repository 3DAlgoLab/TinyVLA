...
def training_data(res, *, final_split, text_len=32):
    ...
    c.data = dict(
        name="ocrvqa_id",
        split=(
            "train+val" if final_split else "train + val[20_000:]"
        ),  # Val is 100k, we don't need that much!
    )
    c.pp = "|".join(
        [
...
def add_eval(c, res, text_len=32, **kw):
    ...
    for freq, name, split in [
        ...
        c.evals[f"ocrvqa/{name}"] = dict(
            ...
            pred="decode",
            pred_kw={"max_decode_len": text_len},
            data={
                **training_data(res, final_split=True, text_len=text_len).data,
                "split": split,
            },
            log_percent=freq,
            skip_first=freq == 1,
...
def add_eval_pplx(c, res, text_len=32):
    ...
    c_train = training_data(
        res, final_split=True, text_len=text_len
    )  # Use mostly same settings as training.
    for name, split in [
        ...
        c.evals[f"ocrvqa/{name}/pplx"] = dict(
            type="proj.paligemma.perplexity",
            pred="logits",
            key="text",
            shift_labels=True,
