...
def get_config(arg=None):
    ...
    arg = bvcc.parse_arg(
        arg, runlocal=False, data="food", variant="medium", crop="inception_crop(128)"
    )
    ...
    config.distance = "kl"
    config.distance_kw = dict(
        t={
            "food": {"fast": 10.0, "medium": 10.0, "long": 5.0},
            "sun": {"fast": 10.0, "medium": 10.0, "long": 10.0},
    ...
    return hyper.product(
        [
            ...
            hyper.chainit(
                ...
                + [
                    setting(p=p, l=l, m=m, crop=f"inception_crop({L})")
                    for p, l, m in plm
                ],
            ),
        ]
