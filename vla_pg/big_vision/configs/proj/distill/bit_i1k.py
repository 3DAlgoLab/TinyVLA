...
def get_config(arg=None):
    ...
    config.input.pp = "decode_jpeg_and_inception_crop(224)|flip_lr" + pp_common.format(
        lbl="label"
    )
    ...
    config.evals.cka_train = get_cka(
        "train[:24576]" if not arg.runlocal else "train[:16]"
    )
    config.evals.cka_minival = get_cka(
        "train[-24576:]" if not arg.runlocal else "train[:16]"
    )
    config.evals.cka_val = get_cka(
        "validation[:49152]" if not arg.runlocal else "validation[:16]"
    )
