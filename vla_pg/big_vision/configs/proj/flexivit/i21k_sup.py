...
def get_config(arg=None):
    ...
    pp_common_i1k = pp_common.format(
        onehot_args='1000, key="{lbl}", key_result="labels"'
    )
    c.input.pp = (
        f"decode_jpeg_and_inception_crop({c.res})|flip_lr|randaug(2,10)"
        + pp_common_i21k
    )
    ...
    for s in c.flexi.seqhw.v:
        c.evals[f"i1k_val{s:02d}"] = eval_i1k(
        ...
        c.evals[f"i1k_real{s:02d}"] = eval_i1k(
            s, "imagenet2012_real", "validation", "i1k_i21k"
        )
        c.evals[f"i1k_real{s:02d}"].pp_fn = pp_eval() + pp_common_i1k.format(
            lbl="real_label"
        )
        # TODO: add objectnet.
