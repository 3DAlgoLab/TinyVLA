...
def apply_mask_schedule(ratio: float | jax.Array, method: str) -> jax.Array:
    ...
    if method == "cosine":
        mask_ratio = jax.lax.cos(jnp.pi / 2.0 * ratio)
    elif "pow:" in method:
    ...
    mask_ratio = jnp.clip(mask_ratio, 1e-6, 1.0)
    return mask_ratio
...
class EncoderDecoderBlock(nn.Module):
    ...
    def __call__(
        ...
        z = wlc(
            vit.MlpBlock(mlp_dim=self.mlp_dim, dropout=self.dropout_rate, name="MLP")(
...
class Decoder(nn.Module):
    ...
    def __call__(
        ...
        pos_emb = vit.get_posemb(self, "learn", self.seq_len, self.emb_dim, "pos_emb")

        y = common.AddPositionEmbs(decode=decode, name="PosEmbedTargets")(
            targets, pos_emb
        )
        ...
        if self.scan:
            ...
            out = {
                f"block{l}_rep": jnp.mean(out[l], axis=1)
            ...
            for lyr in range(self.num_layers):
                ...
                )(y, encoded, decoder_mask=decoder_mask, deterministic=deterministic)
...
class Model(nn.Module):
    ...
    def setup(self) -> None:
        ...
        if self.num_layers > 0:
            ...
            self.pos_emb_for_encoder = vit.get_posemb(
                self, self.posemb_type, grid_size, self.emb_dim, "pos_embedding_encoder"
            )
    ...
    def _get_special_label(self, size, label: _SpecialLabel):
        return self.labels_emb(jnp.full(size, self.special_labels[label], jnp.int32))
    ...
    def _decode_masked(
        ...
        # (B, L+1, E).
        targets_embedded = jnp.concatenate([labels_embedded, targets_embedded], axis=1)
    ...
    def _drop_labels(self, drop_labels_mask, labels):
        ...
        nolabel = jnp.full_like(labels, self.special_labels[_SpecialLabel.NOLABEL])
    ...
    def decode(
        ...
        labels = self._drop_labels(drop_labels, labels)
        if self.style == "ar":
            logits, out = self._decode_ar(targets, labels, encoded, decode, train)
            if return_reps:
                return logits, out
            return logits
        elif self.style == "masked":
    ...
    if isinstance(init_files, str):
        ...
        if trim_decoder_posemb:
            if init_params and "pos_embedding_decoder" in init_params:
                ckpt_params["pos_embedding_decoder"] = ckpt_params[
        ...
        if enc_init:
            ...
            encoder_params = vit.load(
                vit_params, enc_init, model_cfg={}, dont_load=dont_load
            )
