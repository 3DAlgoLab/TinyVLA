...
class Model(vae.Model):
    ...
    def setup(self) -> None:
        ...
        self.head = nn.Dense(
            num_out_channels * np.prod(self.patch_size), name="decoder_head", **kw
        )
    ...
    def encode(
        ...
        *,
        ...
        if self.bottleneck_resize:
            x = einops.rearrange(
                x, "b (h w) c -> b h w c", h=self.grid_size[0], w=self.grid_size[1]
            )
            l = int(np.round(self.code_len**0.5))
            x = jax.image.resize(x, (x.shape[0], l, l, x.shape[3]), method="linear")
            x = einops.rearrange(x, "b h w c -> b (h w) c")
        else:
