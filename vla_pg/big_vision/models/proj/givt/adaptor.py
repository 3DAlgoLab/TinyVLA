...
class IRevNetBlock(nn.Module):
    ...
    def _fx2(self, x: jax.Array, train: bool = True) -> jax.Array:
        ...
        y = nn.Conv(
            self.num_channels_bottleneck or self.num_channels,
            kernel_size=ks,
            padding=1,
            use_bias=False,
        )(y)
    ...
    def inverse(
        self, x: tuple[jax.Array, jax.Array], train: bool = True
    ) -> tuple[jax.Array, jax.Array]:
...
class Model(IRevNet):
    ...
    def inverse(self, out_bij: jax.Array, train: bool = True) -> jax.Array:
        # (b, code_len, ch) --> (b, h, w, ch) --> (b, code_len, ch)
        h, w = cnn.get_h_w_pixelshuffle(out_bij.shape[1], self.pixel_shuffle_patch_size)
