...
GenerateFn = Callable[..., Tuple[jnp.ndarray, jnp.ndarray, Optional[jnp.ndarray]]]
    ...
    **generate_fn_kwargs,
    ...
    if prefill:
        cache = nn.apply(prefill_cache, model, mutable=True)(
            {"params": params["params"], "cache": cache}, encoded_inputs, prompts
        )[1]["cache"]

    def tokens_to_logits(tokens, cache):
        def decode_step(model, tokens):
            encoded = expand_samples_dim_and_flatten(encoded_inputs, num_samples)
...
def _init_state(prompts, cache, init_rng_key, num_samples):
    ...
    cache = cache_map(lambda x: expand_samples_dim_and_flatten(x, num_samples), cache)
    return LoopState(
        cur_index=0,
        cache=cache,
...
def _temperature_sampling_iteration(
    state, tokens_to_logits, temperature, eos, top_k, top_p, mask_token_ids=()
):
    ...
    assert logits.ndim == 2, (
        "tokens_to_logits expected to return a"
    ...
    if top_k:
        ...
        sampled_tokens = jnp.squeeze(
            jnp.take_along_axis(topk_indices, jnp.expand_dims(topk_token, -1), axis=-1),
            axis=-1,
        )
    else:
        sampled_tokens = jax.random.categorical(rng_sampling, logits / temperature)

    sampled_logprobs = jnp.squeeze(
        jnp.take_along_axis(logprobs, jnp.expand_dims(sampled_tokens, axis=1), axis=-1),
        axis=-1,
    )
    ...
    next_logprobs = jnp.squeeze(
        jnp.take_along_axis(logprobs, jnp.expand_dims(next_tokens, axis=1), axis=-1),
        axis=-1,
    )
    ...
    flags_finished = out_of_prompt & (state.flags_finished | (sampled_tokens == eos))
    return LoopState(
...
def _temperature_sampling(
    ...
    should_temperature_sampling_continue_fn = functools.partial(
        _should_temperature_sampling_continue, max_decode_len=max_decode_len + 1
    )  # Account for prompt padding with 0's.
