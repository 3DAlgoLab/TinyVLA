...
class Decoder(nn.Module):
    ...
    def __call__(
        ...
        y = common.AddPositionEmbs(decode=decode, name="PosEmbedTargets")(y, pos_emb)
...
class Model(nn.Module):
    ...
    def decode(
        self, encoded, targets, decode=False, train=False, max_decode_length=None
    ):
    ...
    def __call__(
        self, image, text, *, decode=False, train=False, return_enc_features=False
    ):
        """Applies Transformer model on the inputs.
...
def load(
    ...
    if isinstance(init_files, str):
        ...
        if (
            model_params.get("scan")
            and "encoderblock" not in ckpt_params["encoder"]["Transformer"]
        ):
            raise NotImplementedError(
                "Loading a non-scan checkpoint into a "
                "scan model is not supported yet!"
            )
        if (
            not model_params.get("scan")
            and "encoderblock" in ckpt_params["encoder"]["Transformer"]
        ):
            assert (
                "decoder.*" in dont_load or "decoder/.*" in dont_load
            ), "Converting scan decoder to a non-scan one is not supported yet!"
            ckpt_params["encoder"] = utils.jit_cpu()(vit.scan_to_pyloop)(
                ckpt_params["encoder"]
            )
