...
CKPT = flags.DEFINE_string("ckpt", default=None, help="Path to checkpoint.")
IMAGE = flags.DEFINE_string("image", default=None, help="Path to input image.")

SAMPLER = flags.DEFINE_string(
    "sampler", default="greedy", help="Decoding strategy. Try `nucleus(0.1)`"
)
RES = flags.DEFINE_integer("res", default=224, help="Image resolution (224, 448, 896).")
...
def load_model(ckpt):
    model_cfg = ml_collections.FrozenConfigDict(
        dict(
...
def main(argv):
    ...
    pp_fn = big_vision.pp.builder.get_preprocess_fn(
        "|".join(
            [
    ...
    decode = functools.partial(
        predict_fns["decode"],
        devices=jax.devices(),
        eos_token=tokzr.eos_token,
        max_decode_len=MAX_DECODE_LEN.value,
