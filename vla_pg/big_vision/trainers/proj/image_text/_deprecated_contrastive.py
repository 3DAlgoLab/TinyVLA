...
def _gather_from_device(x, device_id, axis_name="batch"):
    return jax.lax.psum((jax.lax.axis_index(axis_name) == device_id) * x, axis_name)
    ...
    def update_fn(params, opt, rng, batch):
        ...
        def loss_fn(params, images, labels):
            ...
            match config.get("loss_fn", "softmax"):
                ...
                case "sigmoid":
                    l, l_extras = sigmoid_loss(
                        zimg, ztxt, extras["t"], bias=extras["b"]
                    )
                case "chunked_sigmoid":
                    l, l_extras = chunked_sigmoid_loss(
                        zimg, ztxt, extras["t"], bias=extras["b"]
                    )
                case _:
        ...
        (l, measurements), grads = jax.value_and_grad(loss_fn, has_aux=True)(
            params, batch["image"], batch["labels"]
        )
        l, measurements, grads = jax.lax.pmean(
            (l, measurements, grads), axis_name="batch"
        )
