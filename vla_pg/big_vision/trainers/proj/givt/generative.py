...
def main(argv):
    ...
    info(
        "Running for %d steps, that means %f epochs",
        total_steps,
    ...
    vae_mod = importlib.import_module(f"big_vision.models.{config.vae.model_name}")
    ...
    if config.get("adaptor_name"):
        ...
        adaptor = adaptor_mod.Model(
            num_channels=model_config.out_dim, **config.adaptor.model
        )
    else:
        adaptor = None
    ...
    def init(rng):
        ...
        code_shape = (local_batch_size, model_config.seq_len, model_config.out_dim)
        ...
        params = model.init(
            rng, dummy_code, dummy_labels, image=dummy_cond_img, input_mask=input_mask
        )["params"]
    ...
    params_vae = vae_mod.load(
        None, config.vae.model_init, **config.vae.get("model_load", {})
    )
    ...
    def vae_encode(images, rng=None, reparametrize=True):
        ...
        if reparametrize:
            ...
            return vae.apply(
                {"params": params_vae}, mu, logvar, method=vae.reparametrize, rngs=rng
            )
        return mu
    ...
    def loss_fn(params, images, labels, cond_images, rng):
        ...
        if adaptor is not None:
            ...
            sequence = adaptor.apply(
                {"params": params["params_adaptor"]}, sequence, method=adaptor.forward
            )
        ...
        if input_mask is not None:
            metrics["fraction_masked_out"] = input_mask.astype(jnp.float32).mean(axis=1)
            if nll.ndim == 3:
    ...
    def update_fn(train_state, rng, batch):
        ...
        gs = jax.tree_leaves(bv_optax.replace_frozen(config.schedule, grads, 0.0))
        ...
        if adaptor is not None:
            ...
            measurements["l2_params_adaptor"] = jnp.sqrt(
                sum([jnp.vdot(p, p) for p in ps_a])
            )
    ...
    def predict_fn_teacher_forcing(train_state, batch, seed=0):
        ...
        if adaptor is not None:
            ...
            sampled_sequence = adaptor.apply(
                {"params": params["params_adaptor"]},
                sampled_sequence,
                method=adaptor.inverse,
            )
        logits = vae.apply({"params": params_vae}, sampled_sequence, method=vae.decode)
    ...
    def predict_fn_sampling(train_state, batch, seed=0):
        ...
        if model.style == "ar":
            ...
            sampled_codes = (
                parallel_decode.decode_masked(  # pytype: disable=wrong-arg-types
                    rng=local_rng,
                    labels=labels,
                    seq_len=config.model.seq_len,
                    feature_dim=config.model.out_dim,
    ...
    def predict_fn_sampling_panoptic(train_state, batch, seed=0, min_fraction=0.0):
    ...
    if resume_ckpt_path:
        ...
        shardings = {
            **train_state_sharding,
            "chrono": jax.tree_map(lambda _: repl_sharding, u.chrono.save()),
        }
    ...
    for step, batch in zip(range(first_step + 1, total_steps + 1), train_iter):
        ...
        if (
            u.itstime(step, get_steps("log_training"), total_steps, host=0)
            or u.chrono.warmup
