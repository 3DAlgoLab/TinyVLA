...
def main(argv):
    ...
    for m in config.get(
        "pp_modules",
        ["ops_general", "ops_image", "proj.uvim.pp_ops", "proj.givt.pp_ops"],
    ):
    ...
    def vae_loss_fn(logits, image, mu, logvar, beta=1.0, keep_batch_dim=False):
        ...
        loss_kl = -0.5 * jnp.sum(
            1 + logvar - jnp.square(mu) - jnp.exp(logvar), axis=tuple(range(1, mu.ndim))
        )
    ...
    def validation_fn(train_state, batch, seed=0):
        ...
        return jax.tree_map(
            lambda x: jnp.mean(x, axis=tuple(range(1, x.ndim))), aux_loss
        )
    ...
    def predict_fn_depth(train_state, batch):
        depth = predict_fn({"params": train_state["params"]}, batch)["logits"]["depth"]
