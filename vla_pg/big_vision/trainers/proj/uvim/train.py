...
def main(argv):
    ...
    for m in config.get("pp_modules", ["ops_general", "ops_image", "proj.uvim.pp_ops"]):
    ...
    encode_labels, decode_labels, predict_outputs_fn, task_params = setup_task(config)
    ...
    def init(rng):
        batch = jax.tree_map(
            lambda x: jnp.zeros(x.shape, x.dtype.as_numpy_dtype), train_ds.element_spec
        )
        images = batch["image"]
    ...
    def update_fn(params, opt, batch, update_rng, task_params):
        ...
        def loss_fn(params, images, labels):
            ...
            loss = u.weighted_softmax_xent(
                logits=logits, labels=labels, reduction=True, normalize=True
            )
            return loss
    ...
    def validation_fn(params, batch):
        ...
        loss = u.weighted_softmax_xent(
            logits=logits, labels=labels, reduction=False, normalize=True
        )
        losses = {"loss": loss}
        return jax.tree_map(lambda x: jnp.mean(x, axis=tuple(range(1, x.ndim))), losses)
    ...
    def evaluators():
        return eval_common.from_config(
            config,
            {"predict": predict_fn, "validation": validation_fn},
    ...
    if resume_ckpt_path:
        ...
        params_cpu = model_mod.load(
            params_cpu, config.model_init, config.model, **config.get("model_load", {})
        )
        if jax.process_index() == 0:
    ...
    for step, batch in zip(range(first_step + 1, total_steps + 1), train_iter):
        ...
        with jax.profiler.StepTraceAnnotation("train_step", step_num=step):
            params_repl, opt_repl, loss_value, update_rngs, measurements = update_fn(
                params_repl,
                opt_repl,
                batch,
        ...
        if save_ckpt_path and (
            ...
            or u.itstime(step, get_steps("keep_ckpt", None), total_steps, host=0)
        ):
        ...
        for name, evaluator, log_steps, prefix in evaluators():
            if u.itstime(
                step, log_steps, total_steps, first=log_steps < total_steps, last=False
            ):
