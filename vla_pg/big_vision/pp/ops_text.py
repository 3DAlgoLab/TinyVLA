...
KNOWN_TOKENIZERS = {
    "mc4": "gs://t5-data/vocabs/mc4.250000.100extra/sentencepiece.model",  # used in multilingual models (mT5, PaLI), vocab_size=250_000
    "cc_all": "gs://t5-data/vocabs/cc_all.32000/sentencepiece.model",  # vocab_size=32_000
    "c4_en": "gs://t5-data/vocabs/cc_en.32000/sentencepiece.model",  # vocab_size=32_000
    "t5": "gs://t5-data/vocabs/cc_all.32000.100extra/sentencepiece.model",  # same as cc_all, but with 100 extra dummy tokens used by T5 models
    "mt5": "gs://t5-data/vocabs/mc4.250000.100extra/sentencepiece.model",  # same as mc4, but with 100 extra dummy tokens used by T5 models
}
...
def tokenize(input_text, tokenizer, max_len, *, pad_value, force_eos, multi_text=False):
        ...
        if force_eos:
            tokens = tf.cond(
                ...
                lambda: tf.concat(
                    ...
                    [tokens[: max_len - 1], tokens[-1:]],
                    axis=0,
                ),
                lambda: tf.pad(
                    tokens,
                    [(0, max_len - tf.shape(tokens)[0])],
            ...
            tokens = tf.pad(
                tokens, [(0, max_len - tf.shape(tokens)[0])], constant_values=pad_value
            )
...
def get_pp_tokenize(
    ...
    def _pp_tokenize(txt):
        if sample_if_multi and tf.convert_to_tensor(txt).ndim:
            ...
            logging.warning(
                "sample_if_multi is deprecated and will be removed."
                "Call `choice` (and maybe `setdefault`) instead."
            )
            txt = ops_general.get_choice(key="t")(
                ops_general.get_setdefault("t", "")({"t": txt})
            )["t"]
        ...
        if lower:
            txt = (
                ...
                if sample_if_multi
                else tf.map_fn(tf.strings.lower, txt)
            )
...
def _add_pieces(model_bytes, extra_pieces):
    ...
    updated_pieces = [
        idx_to_updated_piece.get(i, piece) for i, piece in enumerate(model_proto.pieces)
    ]
