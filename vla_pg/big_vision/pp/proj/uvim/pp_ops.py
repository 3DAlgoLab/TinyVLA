...
def get_coco_panoptic_pp():
    ...
    def _coco_panoptic(data):
        ...
        n_masks = tf.cast(id_mask[:, :, None] == instance_ids[None, None, :], tf.int32)
...
def get_make_canonical(random=False, main_sort_axis="y"):
    ...
    def _make_canonical(image):
        ...
        ids = ids[ids > 0]
        n_masks = tf.cast(instimg[None, :, :] == ids[:, None, None], tf.int32)
...
def get_strong_hash():
    ...
    def _strong_hash(string):
        return tf.strings.to_hash_bucket_strong(
            string, np.iinfo(int).max, [3714561454027272724, 8800639020734831960]
        )
