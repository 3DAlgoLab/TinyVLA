...
class PreprocessOpsTest(tf.test.TestCase):
    ...
    def test_det_flip_lr(self):
        ...
        for dtype in [tf.uint8, tf.float32]:
            ...
            for out in self.tfrun(
                pp.get_det_fliplr(randkey="rand"), {"rand": 0.1, **image_data}
            ):
                self.assertTrue(np.all(image_data["image"] == out["image"]))
                self.assertEqual(out["image"].dtype, dtype)
            for out in self.tfrun(
                pp.get_det_fliplr(randkey="rand"), {"rand": 0.6, **image_data}
            ):
    ...
    def test_crop_box(self):
        ...
        for out in self.tfrun(pp.get_crop_box(), data):
            ...
            self.assertAllEqual(
                data["image"][320 : 320 + 160, 128 : 128 + 96], out["image"]
            )

    def test_make_canonical(self):
        orig = np.array(
            [[1, 0, 3, 3, -1], [1, 0, 3, 3, -1], [1, 0, 2, 2, 2], [1, 0, 0, -1, -1]],
            np.int32,
        )[:, :, None]
        expected = np.array(
            [[2, 0, 1, 1, -1], [2, 0, 1, 1, -1], [2, 0, 3, 3, 3], [2, 0, 0, -1, -1]],
            np.int32,
        )[:, :, None]
        ...
        for out in self.tfrun(
            pp.get_make_canonical(), {"labels": tf.tile(orig, (1, 1, 3))}
        ):
