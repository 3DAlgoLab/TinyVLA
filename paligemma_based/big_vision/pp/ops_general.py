...
def get_onehot(depth, key="labels", key_result=None, multi=True, on=1.0, off=0.0):
...
def get_pad_to_shape(shape, pad_value=0, where="after"):
    ...
    def _pad_to_shape(x):
        ...
        paddings = [_pads(tgt=shape[i], cur=tf.shape(x)[i]) for i in range(len(shape))]
...
def get_choice_no_replacement(key=None, inkey=None, outkey=None):
    ...
    def _choice(data):
        ...
        checks = [
            tf.debugging.assert_equal(tf.shape(data[k])[0], nitems) for k in inkeys
        ]
        with tf.control_dependencies(checks):
            nitems = tf.identity(nitems)
