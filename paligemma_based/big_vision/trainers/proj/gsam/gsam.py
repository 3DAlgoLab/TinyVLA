...
def gsam_gradient(
    ...
    if adaptive_perturbation:
        param_sam = jax.tree_map(
            lambda a, b: a + jnp.abs(a) * sam_rho * b / (g_clean_length + eps),
            params,
            g_clean,
        )
    else:
        param_sam = jax.tree_map(
            lambda a, b: a + sam_rho * b / (g_clean_length + eps), params, g_clean
        )
    ...
    if minimize_fp:
        ...
        g_clean_projection_norm = sum(
            jnp.vdot(p, q)
            for (p, q) in zip(g_robust_normalized_flatten, g_clean_flatten)
        )
        g_clean_residual = jax.tree_map(
            lambda a, b: a - g_clean_projection_norm * b, g_clean, g_robust_normalized
        )

        # Get GSAM gradient.
        g_gsam = jax.tree_map(lambda a, b: a - b * alpha, g_robust, g_clean_residual)
    else:
        ...
        g_robust_projection_norm = sum(
            jnp.vdot(p, q)
            for (p, q) in zip(g_clean_normalized_flatten, g_robust_flatten)
        )
        g_robust_residual = jax.tree_map(
            lambda a, b: a - g_robust_projection_norm * b, g_robust, g_clean_normalized
        )

        # Get GSAM gradient.
        g_gsam = jax.tree_map(lambda a, b: a + b * alpha, g_clean, g_robust_residual)
