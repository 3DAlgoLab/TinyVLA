...
class RefCocoBv(tfds.core.GeneratorBasedBuilder):
  ...
  """
    ...
    def _split_generators(self, dl_manager: tfds.download.DownloadManager):
        ...
        return {
            split
            + suffix: self._generate_examples(
                coco,
                data_dir,
                bc.dataset,
                bc.dataset_partition,
                split + suffix,
            )
            for split in splits
            for suffix in ("", "_flat")
    ...
    def _generate_examples(self, coco, data_dir, dataset, dataset_partition, split):
...
def _get_ids(data_dir, dataset, dataset_partition, split):
    ...
    if (dataset, dataset_partition) == ("refcocox", "combined"):
        refcoco = (
            load("refcocog", "umd") + load("refcoco", "unc") + load("refcoco+", "unc")
        )
        if split == "test":
        ...
        if split == "train":
            ...
            exclude_img_ids = {r["image_id"] for r in refcoco if r["split"] != "train"}
        refcoco = [
            r
            for r in refcoco
    ...
    for r in refcoco:
        for sent in r["sentences"]:
            ann_to_refs.setdefault(r["ann_id"], []).append(
                dict(id=sent["sent_id"], sentence=sent["sent"])
            )
