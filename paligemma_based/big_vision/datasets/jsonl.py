...
class DataSource(ds_core.DataSource):
    ...
    def __init__(
        self, fname, *, fopen_keys=(), download_keys=(), start=0, stop=float("inf")
    ):
    ...
    def get_tfdata(self, ordered=False, *, process_split=True, allow_cache=True):
        ...
        assert (
            not process_split or len(self.examples) >= jax.process_count()
    ...
    def num_examples_per_process(self):
        return [
            len(self._indices(process_index=pid)) for pid in range(jax.process_count())
        ]
