    ...
    def __call__(self, image, texts):
        ...
        def z(x):
            if x is not None:
                ...
                x = jnp.concatenate(
                    [100 * jnp.ones([batch_size, 1]), x[:, None]], axis=1
                )
...
class RetrievalTest(tf.test.TestCase):
    ...
    def test_prepare_datasets(self):
        ...
        def generator():
            yield {
                ...
                "captions": {"text": tf.constant(["11", "12"])},
            }
            yield {
                "image": tf.ones([4, 4, 3], tf.float32),
                "captions": {"text": tf.constant(["21", "22", "23"])},
            }
    ...
    def test_evaluate(self):
        ...
        with tfds.testing.mock_data(num_examples=num_examples):
            ...
            with mock.patch.object(retrieval, "_get_dataset_info", lambda _: info_mock):
                with registry.temporary_ops(copy_from=_get_copy_from):
                    evaluator = retrieval.Evaluator(
                        lambda p, b: model.apply(
                            {"params": p}, b.get("image", None), b.get("labels", None)
                        ),
