...
def prepare_datasets(dataset, *, pp_img, pp_txt, txt_name, offset=0, cache_final=False):
    ...
    ds_images = (
        dataset.enumerate()
        .map(add_id)
        .map(pp_builder.get_preprocess_fn(f"{pp_img}|keep('id', 'image')"))
    )
    ds_texts = (
        dataset.enumerate()
        .flat_map(get_captions)
        .map(
            pp_builder.get_preprocess_fn(f"{pp_txt}|keep('id', 'caption_i', 'labels')")
        )
    )
    if cache_final:
...
class Evaluator:
    ...
    def __init__(
        ...
        *,
        ...
        self._embed_images_p = jax.jit(
            embed_images, out_shardings=NamedSharding(mesh, P())
        )
        self._embed_texts_p = jax.jit(
            embed_texts, out_shardings=NamedSharding(mesh, P())
        )
    ...
    def evaluate(self, train_state):
        ...
        images = self._embed(
            "image", train_state, self.ds_images, self._embed_images_p, ("id",)
        )
        texts = self._embed(
            "labels",
            train_state,
