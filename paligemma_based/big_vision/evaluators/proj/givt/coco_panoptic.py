...
class Evaluator:
    ...
    def __init__(
        ...
        *,
        ...
        data_specs = dict(name="coco/2017_panoptic", data_dir=None, split="validation")
        ...
        self.data_iter = input_pipeline.start_global(self.dataset, devices, prefetch)
        ...
        if jax.process_index() == 0:
            ...
            (
                ...
                data_specs["name"], data_specs["split"], data_specs.get("data_dir")
            )
            if save_dir:
    ...
    def _compute_png_predictions(self, train_state: Any) -> Any:
        ...
        for batch in itertools.islice(self.data_iter, self.steps):
            ...
            logging.log_every_n_seconds(
                logging.INFO, "Panoptic eval: processed %i examples so far.", 30, count
            )
    ...
    def run(self, train_state):
        ...
        with tempfile.TemporaryDirectory() as pred_folder, tempfile.NamedTemporaryFile(
            mode="w"
        ) as pred_json:
            ...
            res = evaluation.pq_compute(
                gt_json_file=self.gt_json,
...
def _prepare_ground_truth_from_dataset(dataset, split, data_dir):
    ...
    """
    tfds_dataset = tfds.builder(dataset, data_dir=data_dir).as_dataset(split=split)
...
def _prepare_ground_truth_from_zipfiles(split):
    ...
    gt_folder = os.path.join(
        gt_folder,
        "panoptic_val2017" if split_prefix == "validation" else "panoptic_train2017",
    )
    ...
    with gfile.GFile(gt_json) as f:
        data = json.load(f)
    logging.info("Panoptic eval: pre-filter %d annotations.", len(data["annotations"]))
    data["images"] = [x for x in data["images"] if x["id"] in image_ids]
    data["annotations"] = [x for x in data["annotations"] if x["image_id"] in image_ids]
    logging.info("Panoptic eval: post-filter %d annotations.", len(data["annotations"]))
