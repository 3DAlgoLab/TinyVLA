...
class Evaluator:
    ...
    def __init__(
        ...
        *,
        ...
        self.repr_fn = self.get_representation_fn(predict_fn, representation_layer)
    ...
    def _get_repr(self, params, data, steps):
        ...
        for batch, _ in zip(
            ...
            pre_logits, labels, mask = jax.device_get(
                self.repr_fn(params, batch, labels, mask)
            )
    ...
    def compute_fewshot_metrics(
        self, train_state, seed, dataset, train_split, test_split
    ):
        """Compute few-shot metrics on one dataset."""
        if dataset in self._repr:
            repr_train, labels_train, repr_test, labels_test, num_classes = self._repr[
                dataset
            ]
        else:
        ...
        class_indices = [
            rng.permutation(np.where(labels_train == cls_i)[0])
