...
def get_fewshot_lsr(target_resolution=224, resize_resolution=256, runlocal=False, **kw):
    ...
    if not any(
        f"log_{x}" in kw for x in ["steps", "percent", "examples", "epochs"]
    ...
    config.pp_train = (
        f"decode|resize({resize_resolution})|"
