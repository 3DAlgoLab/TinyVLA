...
iou_fn_batched = jax.vmap(jax.vmap(iou_fn, in_axes=(None, 0)), in_axes=(0, None))


def _reward_fn_thr(
    seq_pred, seq_gt, thr, nms_w, max_level, max_conf, num_cls, cls_smooth
):
    ...
    boxes_gt, labels_gt, confs_gt = seq2box(seq_gt, max_level, max_conf, num_cls)
...
def reward_fn(seqs_pred, seqs_gt, config):
    ...
    for thr in thrs:
        ...
        result = {**result, **{f"{k}-{thr:0.1f}": rewards[k] for k in r_keys}}
        if thr == correct_thr:
    ...
    result = {
        **result,
        **{
            k: jnp.mean(jnp.array([result[f"{k}-{thr:0.1f}"] for thr in thrs]), axis=0)
            for k in r_keys
        },
    }
