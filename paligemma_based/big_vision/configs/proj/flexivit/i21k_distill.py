...
def get_config(arg=None):
    ...
    c.input.pp = (
        ...
        f'|resize(224, outkey="prof")|value_range(-1, 1, key="prof")' + pp_label_i21k
    )
    pp_eval_both = (
    ...
    for s in c.flexi.seqhw.v:
        c.evals[f"student_i1k_val{s:02d}"] = eval_i1k(
            s, "imagenet2012", "validation", "i1k_i21k"
        )
        c.evals[f"student_i1k_v2{s:02d}"] = eval_i1k(
            s, "imagenet_v2", "test", "i1k_i21k"
        )
        c.evals[f"student_i1k_a{s:02d}"] = eval_i1k(
            s, "imagenet_a", "test", "i1ka_i21k"
        )
        c.evals[f"student_i1k_r{s:02d}"] = eval_i1k(
            s, "imagenet_r", "test", "i1kr_i21k"
        )
        c.evals[f"student_i1k_real{s:02d}"] = eval_i1k(
            s, "imagenet2012_real", "validation", "i1k_i21k"
        )
        c.evals[f"student_i1k_real{s:02d}"].pp_fn = (
            pp_eval_student + pp_label_i1k.format(lbl="real_label")
        )
        # TODO: add objectnet.
    ...
    c.evals.teacher_i1k_real.pp_fn = pp_eval_prof + pp_label_i1k.format(
        lbl="real_label"
    )
    # TODO: add objectnet.
