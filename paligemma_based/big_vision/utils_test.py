...
class TreeTest(tf.test.TestCase):
    ...
    def setUp(self):
        ...
        self.d2 = {"conv1": {"kernel": 0, "bias": 1}, "conv2": {"kernel": 2, "bias": 3}}
    ...
    def test_traverse_with_names(self):
        ...
        names_and_vals = list(
            utils._traverse_with_names(self.d2, with_inner_nodes=True)
        )
    ...
    def test_recover_tree(self):
        ...
        values = [0, 1, 2, 3]
        self.assertEqual(
            utils.recover_tree(keys, values),
            {"a": {"b": 0, "c": {"x": 1, "y": 2}}, "d": 3},
        )
    ...
    def test_make_mask_trees(self):
        ...
        self.assertEqual(utils.make_mask_trees(tree, (".*/x", "b/.*")), [msk1, msk2])
    ...
    def test_tree_compare(self):
        tree1_only, tree2_only, dtype_shape_mismatch = utils.tree_compare(
            ...
            {"a": {"B": jnp.array(2), "c": jnp.array(3.0)}},
        )
...
class StepConversionTest(parameterized.TestCase, tf.test.TestCase):
    ...
    def test_steps(self, data_size, batch_size, total, cfg, expected):
        ...
        step = utils.steps(
            "foo", cfg, data_size=data_size, batch_size=batch_size, total_steps=total
        )
...
class CreateLearningRateScheduleTest(parameterized.TestCase, tf.test.TestCase):
    ...
    @parameterized.named_parameters(
        ("linear", "linear", {}, 13, 0.5),
        ("polynomial", "polynomial", {"end": 0.1, "power": 2}, 13, 0.325),
        ("cosine", "cosine", {}, 13, 0.5),
        ("rsqrt", "rsqrt", {"timescale": 1}, 13, 0.3333333),
        ("stair_5", "stair", {"steps": [10], "mults": [0.5]}, 5, 1.0),
        ("stair_10", "stair", {"steps": [10], "mults": [0.5]}, 10, 0.5),
        ("warmup_before", "rsqrt", {"timescale": 1}, 3, 0.6),
        ("cooldown_after", "rsqrt", {"timescale": 1}, 20, 0.05),
    )
...
class CheckpointTest(tf.test.TestCase):
    ...
    def test_save_and_load(self):
        ...
        ckpt_loaded = utils.load_checkpoint_ts(
            save_path, tree=ckpt, shardings=shardings
        )
