...
class SeparateBlocksReward(base_reward.LanguageTableReward):
    ...
    def __init__(self, goal_reward, rng, delay_reward_steps, block_mode):
        super(SeparateBlocksReward, self).__init__(
    ...
    def _sample_instruction(self, block, avoid_blocks, blocks_on_table):
        ...
        avoid_syns = [self._get_block_synonym(i, blocks_on_table) for i in avoid_blocks]
    ...
    def _get_closest_blocks(self, block, block_translation, block_translations):
        # Compute distances to other blocks.
        distances = [
            (name, np.linalg.norm(block_translation - translation))
            for (name, translation) in block_translations
            if name != block
        ]
        # Sort distances.
    ...
    def _get_blocks_to_separate(self, state, blocks_on_table):
        ...
        block_translations = [
            (i, self._get_translation_for_block(i, state)) for i in blocks_on_table
        ]
    ...
    def _get_avoid_direction(self, state, push_block, avoid_blocks):
        ...
        avoid_centroid_xy = np.mean(
            [self._get_translation_for_block(i, state) for i in avoid_blocks], axis=0
        )
    ...
    def reward_for_info(self, state, info):
        ...
        return self.reward_for(
            state, push_block=info.block, target_translation=info.target_translation
        )
